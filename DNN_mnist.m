...
function next = forward_query(network_matrix, actual)
    ...
    inner = network_matrix * actual;        # Calculate the signals emerging from actual layer
...
function before = backward_query(network_matrix, actual)
    ...
    inner = transpose(network_matrix) * self_inverse_activation_function(actual); # Calculate the input signals
...
function train(inputs_list, targets_list)
    ...
    hidden_outputs = forward_query(self_wih, inputs);        # Run forward towards layers
...
function inputs = backquery(targets_list)
    ...
    hidden_outputs = backward_query(self_who, final_outputs); # Run backward towards layers       
